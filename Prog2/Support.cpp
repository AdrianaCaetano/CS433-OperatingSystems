...
int support::split_command(char* input, char** cmd) {	
  ...
  temp = strtok(input, " \t\r\a\n"); // tabs, spaces, etc, are used as tokens to separate strings
...
int support::execute_command(char** cmd, int num_arg) {
  ...
  while ( i < num_arg ) {
    ...
    if (strcmp(cmd[i], ">") == 0 || strcmp(cmd[i], "<") == 0) { // Redirection
      ...
      run = support::redirect(cmd, num_arg, separator); //redirection of commands
      return run;
    }
...
int support::execute(char** cmd, bool concurrent) {
  ...
  if (pid < 0 ) { 
    ...
    if (execvp(*cmd, cmd) < 0) {
      ...
      exit(1); // exit if command not found
    }
  } 
  else { 
    // Parent process
